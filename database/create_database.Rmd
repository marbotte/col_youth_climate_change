...
wb2 <- loadWorkbook("../../Data_Document/Base de Datos Jovenes_Model_OddsRatio.xlsx")
names(wb2)
data2 <- readWorkbook(wb2,1)
codeBook2 <- readWorkbook(wb2,2,skipEmptyRows = F,colNames=F)
emptyRows<-which(apply(codeBook2,1,function(x)all(is.na(x))))
```
...
```{r}
beg <- c(0, emptyRows) + 1
end <- c(emptyRows - 1, nrow(codeBook2))
titles<-codeBook2[beg,1]
position <- codeBook2[beg + 2, 3]
etiqueta <- codeBook2[beg + 3, 3]
tipo <- codeBook2[beg +4,3]
medicion<-codeBook2[beg+5,3]
noVal <- (beg+5==end)
#tipo[noVal]
#tipo[!noVal]
...
for(i in 1:length(beg))
{
  if(noVal[i])
  {listVal[[i]] <- NULL}else
  {listVal[[i]] <- codeBook2[(beg[i]+6):(end[i]),2:3]}
...
```{r}
colNotInDic <- (!colnames(data2) %in% descr$title)
dicNotInCol <- (!descr$title %in% colnames(data2))
```
...
```{r}
colnames(data2)[colNotInDic]
head(data2[,colNotInDic])
```

*It appears that the variables which are not in the dictionary are composite variables that may be calculated from the other ones and that were added as new colums to the file...*
*The best way to do it in a database management system is to create a view which calculate these variables.*
...
```{r}
require(kableExtra)
infoTab<-read.csv("descr_question.csv")
kable(infoTab)%>%
...
# Insert administrative limits
pgPostGIS(cc_y)
municipios <- as(gadm("Colombia", level=2, path=tempdir()),"Spatial")
unique(data2$Departamento[!tolower(data2$Departamento)%in%tolower(municipios$NAME_1)])
unique(data2$Municipio[!tolower(data2$Municipio)%in%tolower(municipios$NAME_2)])
