...
stopifnot(require(openxlsx), require(kableExtra), require(dm), require(rsvg), require(DiagrammeRsvg), require(tidyverse), require(rms), require(flextable), require(caret), require(lmtest), require(car), require(yardstick), require(papaja), require(pandoc), require(tinytex), require(RColorBrewer))
...
```{r variable emociones hacia cambio climático, echo=F}
...
El conjunto de 16 emociones fue agrupado en tres niveles: Con Eco-ansiedad, emociones ambivalentes, y emociones negativas.
...
data1$emo_category[i] <- "Con eco-ansiedad"
} else if (neg_emo_count == 1 & pos_emo_count == 1 & amb_emo_count == 0) {
data1$emo_category[i] <- "Ambivalente/Neutral"
...
data1$emo_category[i] <- "Sin eco-ansiedad"
}
}
```


```{r variable emo recodificada, echo=F}
data1$emo_category <- factor(data1$emo_category, levels = c("Con eco-ansiedad", "Ambivalente/Neutral", "Sin eco-ansiedad"))
table_emo_cat <- as.data.frame(table(data1$emo_category)) %>%
  mutate(per = round(Freq / sum(Freq) * 100, 1))
kable(table_emo_cat, col.names = c("Eco_ansiedad", "Frecuencia", "Porcentaje (%)"), booktabs = TRUE)

plot_emo_cat <- data1 %>%
  count(emo_category) %>%
  mutate(prop = (n / sum(n))*100) %>%
  ggplot(aes(x = emo_category, y = prop, fill = emo_category)) +
...
```{r variable emo recodificada 2, echo=F}
data1$emo_category <- factor(data1$emo_category, levels = c("Paralyzing", "Practical", "Absent"))
table_emo_cat <- as.data.frame(table(data1$emo_category)) %>%
  mutate(per = round(Freq / sum(Freq) * 100, 1))
kable(table_emo_cat, col.names = c("Eco-anxiety", "Frecuencia", "Porcentaje (%)"), booktabs = TRUE)
...
data1$emo_category[i] <- "Strong Paralyzing"
...
```{r variable emo recodificada 3, echo=F}
data1$emo_category <- factor(data1$emo_category, levels = c("Strong Paralyzing", "Paralyzing", "Practical", "Absent"))
table_emo_cat <- as.data.frame(table(data1$emo_category)) %>%
  mutate(per = round(Freq / sum(Freq) * 100, 1))
kable(table_emo_cat, col.names = c("Eco-anxiety", "Frecuencia", "Porcentaje (%)"), booktabs = TRUE)
...
El rango de edad permitido para los participantes fue separado en 4 distintos grupos.
```{r variable edad recodificada, echo=F}
data1$age_range <- as.factor(data1$age_range)
table_age_range <- as.data.frame(table(data1$age_range)) %>%
  mutate(per = round(Freq / sum(Freq) * 100, 1))
kable(table_age_range, col.names = c("Rango de edad", "Frecuencia", "Porcentaje (%)"), booktabs = TRUE)

plot_age_range <- data1 %>%
  count(age_range) %>%
  mutate(prop = (n / sum(n))*100) %>%
  ggplot(aes(x = age_range, y = prop, fill = age_range)) +
  geom_bar(stat = "identity") +
  coord_flip() +
...
```{r variable estrato recodificada, echo=F}
data1$stratum <- factor(data1$stratum, levels = c("Bajo", "Medio", "Alto"))
table_stratum <- as.data.frame(table(data1$stratum)) %>%
  mutate(per = round(Freq / sum(Freq) * 100, 1))
kable(table_stratum, col.names = c("Estrato", "Frecuencia", "Porcentaje (%)"), booktabs = TRUE)

plot_stratum <- data1 %>%
  count(stratum) %>%
  mutate(prop = (n / sum(n))*100) %>%
  ggplot(aes(x = stratum, y = prop, fill = stratum)) +
  geom_bar(stat = "identity") +
  labs(x = "Estrato", y = "Porcentaje (%)") +
  ggtitle("Distribución \n Estrato") +
  papaja::theme_apa() +
...
La variable "Etnia" fue recodificada en dos distintos niveles que distinguen si la persona pertenece a alguna de las etnias incluidas o no.
```{r variable etnia recodificada, echo=F}
data1$etnia <- as.factor(data1$etnia)
table_etnia <- as.data.frame(table(data1$etnia)) %>%
  mutate(per = round(Freq / sum(Freq) * 100, 1))
kable(table_etnia, col.names = c("¿Pertenece a alguna etnia?", "Frecuencia", "Porcentaje (%)"), booktabs = TRUE)

plot_etnia <- data1 %>%
  count(etnia) %>%
  mutate(prop = (n / sum(n))*100) %>%
  ggplot(aes(x = etnia, y = prop, fill = etnia)) +
  geom_bar(stat = "identity") +
...
```{r variable nivel educativo, echo=F}
data1$A6 <- as.factor(data1$A6)
data1$high_edu <- data1$A6
levels(data1$high_edu) <- c("Básica Primaria", "Básica Secundaria", "Secundaria Completa", "Técnico o Tecnólogo", "Pregrado", "Posgrado")
table_edu <- as.data.frame(table(data1$high_edu)) %>%
  mutate(per = round(Freq / sum(Freq) * 100, 1))
...
plot_edu <- data1 %>%
  count(high_edu) %>%
  mutate(prop = (n / sum(n))*100) %>%
  ggplot(aes(x = high_edu, y = prop, fill = high_edu)) +
  geom_bar(stat = "identity") +
  coord_flip() +
...
```{r variable nivel educativo recodificada, echo=F}
data1$high_edu <- as.factor(data1$high_edu)
table_high_edu <- as.data.frame(table(data1$high_edu)) %>%
  mutate(per = round(Freq / sum(Freq) * 100, 1))
kable(table_high_edu, col.names = c("¿Educación superior?", "Frecuencia", "Porcentaje (%)"), booktabs = TRUE)
...
```{r variable ideología política, echo=F}
...
La variable "Ideología política" fue recodificada en dos distintos niveles que distinguen si la persona se considera de izquierda o derecha.
```{r variable ideología política recodificada, echo=F}
data1$pol_ide <- factor(data1$pol_ide, levels = c("Derecha", "Centro", "Izquierda"))
table_pol_ide <- as.data.frame(table(data1$pol_ide)) %>%
  mutate(per = round(Freq / sum(Freq) * 100, 1))
kable(table_pol_ide, col.names = c("Ideología política", "Frecuencia", "Porcentaje (%)"), booktabs = TRUE)
...
plot_pol_ide <- data1 %>%
  count(pol_ide) %>%
  mutate(prop = (n / sum(n)) * 100) %>%
  ggplot(aes(x = "", y = prop, fill = pol_ide)) +
  geom_col(width = 0.2) +
...
```{r recodificación medios de información, include=FALSE}
...
La variable "Medios de información" fue recodificada en tres distintos niveles que distinguen si la persona solo se informa a través de medios tradicionales, si la persona se informa con medios tanto tradicionales como alternativos, o si la persona solo se informa a través de medios alternativos. Se catalogaron como medios tradicionales la televisión, la radio y la prensa impresa.
```{r variable medios de información recodificada, echo=F}
data1$news <- as.factor(data1$news)
table_news <- as.data.frame(table(data1$news)) %>%
  mutate(per = round(Freq / sum(Freq) * 100, 1))
kable(table_news, col.names = c("Medios de información", "Frecuencia", "Porcentaje (%)"), booktabs = TRUE)

custom_palette <- brewer.pal(3, "Accent")
plot_news <- data1 %>%
  count(news) %>%
  mutate(prop = (n / sum(n))*100) %>%
  ggplot(aes(x = news, y = prop, fill = news)) +
  geom_bar(stat = "identity") +
  labs(x = "Medios de información", y = "Porcentaje (%)") +
  ggtitle("Distribución \n Medios de información") +
  papaja::theme_apa() +
  theme(legend.position="none") +
  geom_text(aes(label = paste0(round(prop, 1), "%")), vjust = -0.5, color = "black", size = 4) +
...
```{r variable redes sociales}
data1$B12_5 <- as.factor(data1$B12_5)
levels(data1$B12_5) <- c("No", "Sí")
table_redes <- as.data.frame(table(data1$B12_5)) %>%
  mutate(per = round(Freq / sum(Freq) * 100, 1))
kable(table_redes, col.names = c("¿Se informa en Redes Sociales?", "Frecuencia", "Porcentaje (%)"), booktabs = TRUE)

plot_redes <- data1 %>%
  count(B12_5) %>%
  mutate(prop = (n / sum(n))*100) %>%
  ggplot(aes(x = B12_5, y = prop, fill = B12_5)) +
  geom_bar(stat = "identity") +
  labs(x = "¿Se informa en Redes Sociales?", y = "Porcentaje (%)") +
  ggtitle("Distribución uso de \n Redes Sociales para informarse") +
  papaja::theme_apa() +
  theme(legend.position="none") +
  geom_text(aes(label = paste0(round(prop, 1), "%")), vjust = -0.5, color = "black", size = 4)
plot_redes
...
```{r, loop medios de información, echo=FALSE}
...
La pregunta número 17 fue: De las siguientes instituciones y figuras, ¿en cuáles confía? Los participantes debían responder para cada una de las 22 figuras e instituciones incluidas si confiaban en esta o no.
```{r confianza en figuras e instituciones, echo=F}
trust_in <- data1 %>%
...
```{r medida desconfianza en figuras e instituciones, eval=FALSE, include=FALSE}
c17 <- grep("^C17_", names(data1), value=T)
c17 <- c17[!grepl("C17_6", c17)]
for (column in c17) {
data1[, column] <- ifelse(data1[, column] == 3, 2, data1[, column])
}
data1$dist_inst <- NA
for (i in 1:nrow(data1)) {
  data1$dist_inst[i] <- sum(data1[i, c17]) - 20
}
```


## Creación de una medida de desconfianza en figuras e instituciones
La creación de la medida de desconfianza en figuras e instituciones se realizó sumando los valores seleccionados para cada una de las figuras e instituciones. De este modo, una persona que confiara en todas las instituciones tendría un puntaje de 21 y una persona que desconfiara de todas las instituciones tendría un puntaje de 42. Las personas que no conocían a la institución o figura y, por tanto, seleccionaron el valor "3" fueron recodificadas con el valor "2", indicando desconfianza. A la suma total obtenida se les restó un valor de 20 para que el rango final de valores de la medida fuera de 1 hasta 22. La única institución no incluida dentro de la medida fue la de Gobernación departamental puesto que esta no aplicaba para una cantidad considerable de la muestra.
```{r variable medida de desconfianza, eval=FALSE, echo=F}
kable(round(table(data1$dist_inst) / length(data1$dist_inst) * 100, digits = 1), col.names = c("Puntaje desconfianza", "Porcentaje (%)"), booktabs = TRUE)

plot_dist <- data1 %>%
  ggplot(aes(x = dist_inst, y = after_stat(count) / sum(after_stat(count)) * 100)) +
  geom_histogram(fill = "#FF9896", binwidth = 1) +
  labs(x = "Desconfianza", y = "Porcentaje (%)") +
  ggtitle("Distribución \n Puntaje Desconfianza") +
  papaja::theme_apa() +
...
```{r variable asociarse a organizaciones ambientales, echo=F}
...
La pregunta número 34 fue: "En una escala de 1 a 6, donde 1 es MUY POCO y 6 es MUY FUERTE, Cuando usted piensa en cómo el gobierno está respondiendo al cambio climático se siente:".
El participante debía dar una respuesta para cada una de las 10 emociones incluidas.
...
```{r variable medida negatividad emocional gobierno, echo=F}
kable(round(table(data1$neg_emo_gov) / length(data1$neg_emo_gov) * 100, digits = 1), col.names = c("Puntaje negatividad emocional", "Porcentaje (%)"), booktabs = TRUE)
...
```{r medida impacto positivo de actores frente al cambio climático, include=FALSE}
d35 <- grep("^D35_", names(data1), value=T)
for (column in d35) {
data1[, column] <- ifelse(data1[, column] == 99, 4, data1[, column])
}
data1$imp_acto <- NA
for (i in 1:nrow(data1)) {
  data1$imp_acto[i] <- sum(data1[i, d35]) - 13
}
```


## Creación de una medida del impacto positivo de distintos actores sobre el cambio climático
La creación de la medida del grado de impacto positivo de distintos actores sobre el cambio climático se obtuvo sumando los puntajes seleccionados para cada uno de los actores. El valor igual a "99" que corresponde a quienes no saben sobre el impacto particular de un actor fue recodificado por el valor "4" indicando aproximadamente un impacto neutro. Al puntaje total obtenido se le restó un valor igual a 13 para que el rango final fuera desde 1 hasta 71.

```{r variable medida impacto positivo actores, echo=F}
plot_imp_acto <- data1 %>%
  ggplot(aes(x = imp_acto, y = after_stat(count) / sum(after_stat(count)) * 100)) +
  geom_histogram(fill = "#E377C2", binwidth = 1) +
  labs(x = "Impacto positivo", y = "Porcentaje (%)") +
  ggtitle("Distribución del grado de impacto positivo de distintos \n actores sobre el cambio climático") +
  papaja::theme_apa() +
  scale_x_continuous(breaks = seq(1, 71, 5))
plot_imp_acto
```
...
```{r variable pensamientos hacia cambio climático, echo=F}
...
La variable "Pensamientos sobre cambio climático" fue recodificada en dos distintos niveles que distinguen si la persona tiene o no mayoritariamente pensamientos negativos.
```{r variable pensamientos negativos cambio climático, echo=F}
data1$th <- as.factor(data1$th)
table_th <- as.data.frame(table(data1$th)) %>%
  mutate(per = round(Freq / sum(Freq) * 100, 1))
kable(table_th, col.names = c("Pensamiento", "Frecuencia", "Porcentaje (%)"), booktabs = TRUE)

plot_th <- data1 %>%
  count(th) %>%
  mutate(prop = (n / sum(n))*100) %>%
  ggplot(aes(x = th, y = prop, fill = th)) +
  geom_bar(stat = "identity") +
  labs(x = "Pensamiento", y = "Porcentaje (%)") +
...
data1$imp_como <- NA
for (i in 1:nrow(data1)) {
  data1$imp_como[i] <- sum(data1[i, c38]) - 9
}
```


## Creación de una medida del impacto positivo de ciertas acciones en la comodidad
A partir de los valores elegidos para cada una de las acciones se creó una medida al sumar todos los valores. El puntaje mínimo es 10 si el impacto de todas las acciones es muy negativo y el puntaje máximo 60 si el impacto es muy positivo para todas. Al total obtenido se le resta un valor de 9 para que el rango vaya desde 1 hasta 51.
```{r variable medida impacto acciones comodidad, echo=F}
kable(round(table(data1$imp_como) / length(data1$imp_como) * 100, digits = 1), col.names = c("Impacto positivo en comodidad", "Porcentaje (%)"), booktabs = TRUE)

plot_imp_como <- data1 %>%
  ggplot(aes(x = imp_como, y = after_stat(count) / sum(after_stat(count)) * 100)) +
  geom_histogram(fill = "#8C564B", binwidth = 1) +
  labs(x = "Impacto positivo", y = "Porcentaje (%)") +
  ggtitle("Distribución puntaje impacto positivo \n de acciones en la comodidad") +
  papaja::theme_apa() +
  scale_x_continuous(breaks = seq(1, 51, 5))
plot_imp_como
```
...
data1$imp_cli <- NA
for (i in 1:nrow(data1)) {
  data1$imp_cli[i] <- sum(data1[i, c39]) - 9
}
```


## Creación de una medida del impacto positivo de ciertas acciones en el cambio climático
A partir de los valores elegidos para cada una de las acciones se creó una medida al sumar todos los valores. El puntaje mínimo es 10 si el impacto de todas las acciones es muy negativo y el puntaje máximo 60 si el impacto es muy positivo para todas. Al total obtenido se le resta un valor de 9 para que el rango vaya desde 1 hasta 51.
```{r variable medida impacto acciones cambio climático, echo=F}
kable(round(table(data1$imp_cli) / length(data1$imp_cli) * 100, digits = 1), col.names = c("Impacto positivo en cambio climático", "Porcentaje (%)"), booktabs = TRUE)

plot_imp_cli <- data1 %>%
  ggplot(aes(x = imp_cli, y = after_stat(count) / sum(after_stat(count)) * 100)) +
  geom_histogram(fill = "#2CA02C", binwidth = 1) +
  labs(x = "Impacto positivo", y = "Porcentaje (%)") +
...
```{r variable origen cambio climático, echo=F}
...
data1 <- data1 %>%
  mutate(clim_ori = case_when(
    C40 >= 1 & C40 <= 5 ~ "Causas naturales",
    C40 >= 6 & C40 <= 10 ~ "Causas humanas"
    ))
```
...
```{r variable origen del cambio climático recodificada, echo=F}
data1$clim_ori <- as.factor(data1$clim_ori)
table_clim_ori <- as.data.frame(table(data1$clim_ori)) %>%
  mutate(per = round(Freq / sum(Freq) * 100, 1))
kable(table_clim_ori, col.names = c("Origen cambio climático", "Frecuencia", "Porcentaje (%)"), booktabs = TRUE)
...
```{r variable contribución factores al cambio climático, echo=F}
...
```{r variable contribución factores recodificada, echo=F}
