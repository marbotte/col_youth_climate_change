...
El conjunto de 16 emociones fue agrupado en tres niveles: Con Eco-ansiedad, emociones ambivalentes y/o neutrales, y emociones negativas.
...
```{r variable medida de confianza, eval=FALSE, echo=F}
...
## Impacto de actores relevantes en el cambio climático
Fueron elegidos los actores considerados como más relevantes frente al fenómeno del cambio climático. Para cada uno se exploró su distribución en relación a su impacto sobre el cambio climático.
```{r medida impacto de actores frente al cambio climático, echo=FALSE}
# Impact of scientists on climate change
data1$imp_scien <- data1$D35_1
data1$imp_scien[data1$imp_scien == 99] <- 3.5
data1$imp_scien <- data1$imp_scien - 3.5

plot_imp_scien <- data1 %>%
  ggplot(aes(x = imp_scien, y = after_stat(count) / sum(after_stat(count)) * 100)) +
  geom_histogram(fill = "#BCBD22", binwidth = 0.5) +
...
data1$imp_act <- data1$imp_act - 3.5

plot_imp_act <- data1 %>%
  ggplot(aes(x = imp_act, y = after_stat(count) / sum(after_stat(count)) * 100)) +
  geom_histogram(fill = "#BCBD22", binwidth = 0.5) +
  labs(x = "Impacto en cambio climático", y = "Porcentaje (%)") +
...
data1$imp_rel <- data1$imp_rel - 3.5

plot_imp_rel <- data1 %>%
  ggplot(aes(x = imp_rel, y = after_stat(count) / sum(after_stat(count)) * 100)) +
  geom_histogram(fill = "#BCBD22", binwidth = 0.5) +
  labs(x = "Impacto en cambio climático", y = "Porcentaje (%)") +
...
data1$imp_pol <- data1$imp_pol - 3.5

plot_imp_pol <- data1 %>%
  ggplot(aes(x = imp_pol, y = after_stat(count) / sum(after_stat(count)) * 100)) +
  geom_histogram(fill = "#BCBD22", binwidth = 0.5) +
  labs(x = "Impacto en cambio climático", y = "Porcentaje (%)") +
