...
plot_cir_mod2 <- data1 %>%
  count(cir_mod2) %>%
  mutate(prop = (n / sum(n))*100) %>%
  ggplot(aes(x = cir_mod2, y = prop, fill = cir_mod2)) +
  geom_bar(stat = "identity") +
  ylim(0, 75) +
  labs(x = "Emociones", y = "Porcentaje (%)") +
  ggtitle("Distribución \n Emociones hacia el cambio climático") +
...
data1 <- data1 %>%
  mutate(neg_emo_gov = case_when(
    neg_emo_gov %in% c(1:10) ~ "Nulo o muy bajo",
    neg_emo_gov %in% c(11:20) ~ "Bajo",
    neg_emo_gov %in% c(21:30) ~ "Medio",
    neg_emo_gov %in% c(31:40) ~ "Alto",
    neg_emo_gov %in% c(41:51) ~ "Muy alto"
))
```
...
```{r variable medida negatividad emocional gobierno, echo=FALSE}
data1$neg_emo_gov <- factor(data1$neg_emo_gov, levels = c("Nulo o muy bajo", "Bajo", "Medio", "Alto", "Muy alto"))
plot_neg_emo_gov <- data1 %>%
  count(neg_emo_gov) %>%
  mutate(prop = (n / sum(n))*100) %>%
  ggplot(aes(x = neg_emo_gov, y = prop, fill = neg_emo_gov)) +
  geom_bar(stat = "identity") +
  ylim(0, 100) +
  labs(x = "Nivel emociones negativas", y = "Porcentaje (%)") +
  ggtitle("Distribución nivel emociones negativas en relación a \nacciones del gobierno en cambio climático") +
  papaja::theme_apa() +
  theme(legend.position="none") +
  geom_text(aes(label = paste0(round(prop, 1), "%")), vjust = -0.5, color = "black", size = 4)
plot_neg_emo_gov
...
```{r variable medida impacto acciones comodidad cont, fig.align='center', echo=F, eval=FALSE}
## Creación de una medida del impacto de ciertas acciones en la comodidad
...
```{r variable medida impacto cambio climático cont, fig.align='center', echo=F, eval=FALSE}
...
```{r medida disposicion acciones en cambio climático, include=FALSE, eval=FALSE}
...
```{r variable medida disposicion acciones en cambio climático, fig.align='center', echo=FALSE, eval=FALSE}
...
```{r medida disposicion acciones en cambio climático otro, include=FALSE}
...
Se creó una medida que promedia los valores asignados a cada una de las acciones individuales y se recodificaron en una escala que va desde -3 hasta 3. Posteriormente, los distintos valores fueron recodificados en una variable con dos niveles, impacto "Negativo" e impacto "Positivo".
```{r variable impacto comodidad recodificada, fig.align='center', echo=FALSE}
table_imp_como <- as.data.frame(table(data1$imp_como)) %>%
  mutate(per = round(Freq / sum(Freq) * 100, 1))
kable(table_imp_como, col.names = c("Impacto en comodidad", "Frecuencia", "Porcentaje (%)"), booktabs = TRUE)
...
plot_imp_como <- data1 %>%
  ...
  geom_bar(stat = "identity") +
  ylim(0, 100) +
  labs(x = "Impacto en comodidad", y = "Porcentaje (%)") +
  ggtitle("Distribución \n impacto en comodidad") +
  papaja::theme_apa() +
  theme(legend.position="none") +
  geom_text(aes(label = paste0(round(prop, 1), "%")), vjust = -0.5, color = "black", size = 4) +
  scale_fill_manual(values = custom_palette)
