...
data1 <- data1 %>%
  mutate(edu = case_when(
    education %in% c("Básica Primaria", "Básica Secundaria", "Secundaria Completa", "Técnico o Tecnólogo", "Pregrado") ~ "No",
    education %in% c("Posgrado") ~ "Sí"
))
...
data2 <- data1 %>%
  select(Zona, sex, edad, estrato, ethni, education, kids, ideology, Televisión, Radio, Prensa_Impresa, Medios_Digitales, Redes_Sociales, trust_vote_inst, eco_anx, env_aso, imp_scien, imp_act, imp_rel, imp_pol, thoughts, imp_como, imp_cli, clim_origin, cont_clim)
...
fviz_mca_ind(res.mca, geom = "point", pointsize = 2, hab = "eco_anx", addEllipses = TRUE, alpha = 0.2)
fviz(res.mca, element = "ind", geom = "point", pointsize = 2, hab = "clim_origin", addEllipses = TRUE, alpha = 0.2)
...
dimdesc(res.mca)
plot(res.mca, invisible = "var", cex = 0.7, label = "none", hab = "clim_origin", select = 0)
...
res.mca <- MCA(data2, quali.sup = c(15:25), graph = FALSE , quanti.sup = 3)
fviz_eig(res.mca, addlabels = TRUE, title = "Inertia of MCA Dimensions")
fviz_mca_ind(res.mca, geom = "point", pointsize = 2)
fviz_mca_ind(res.mca, geom = "point", pointsize = 2, hab = "clim_origin")
...
res.mca <- MCA(data2, quali.sup = c(17, 23), graph = FALSE)
summary(res.mca, ncp = 3, nbelements = Inf)
dimdesc(res.mca)
plot(res.mca, invisible = c("var", "quali.sup"), autoLab = "y", cex = 0.7)
plot(res.mca, invisible = "ind", autoLab = "y", cex = 0.7)
plot(res.mca, invisible = c("ind", "quali.sup"), autoLab = "y", cex = 0.7)
plot(res.mca, invisible = c("ind", "var"), autoLab = "y", cex = 0.7)
plot(res.mca, invisible = "ind", autoLab = "y", cex = 0.7, selectMod = "contrib 20")
```
...
```{r chi-square}
table1 <- table(data2$clim_origin, data2$imp_cli)
table1
chisq.test(table1)

table2 <- table(data2$clim_origin, data2$eco_anx)
table2
chisq.test(table2)

table3 <- table(data2$clim_origin, data2$thoughts)
table3
chisq.test(table3)

table4 <- table(data2$clim_origin, data2$ideology)
table4
chisq.test(table4)

table9 <- table(data2$clim_origin, data2$trust_vote_inst)
table9
chisq.test(table9)

table5 <- table(data2$eco_anx, data2$thoughts)
table5
chisq.test(table5)
...
table9 <- table(data2$eco_anx, data2$imp_como)
table9
chisq.test(table9)
```
...
plot_ori_eco_anx <- data2 %>%
  ggplot(aes(x = eco_anx, fill = clim_origin)) +
  geom_bar(position = "fill") +
  labs(x = "Eco-ansiedad", y = "Proporción", fill = "Origen \nCambio climático") +
  papaja::theme_apa() +
  coord_flip() +
  scale_fill_manual(values = custom_palette)
plot_ori_eco_anx
