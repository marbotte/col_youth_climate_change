...
data2 <- data1 %>%
  select(Zona, sex, age_r, strat, ethni, education, kids, ideology, Televisión, Radio, Prensa_Impresa, Medios_Digitales, Redes_Sociales, Otro_medio, trust_vote_inst, trust_unis, eco_anx, env_aso, imp_scien, imp_act, imp_rel, imp_pol, thoughts, imp_como, imp_cli, clim_origin, cont_clim)
# Not included: Negative emotions towards government actions against climate change, binary coding of education
```
...
res.mca <- MCA(data2, quali.sup = 1:16, graph = FALSE)
summary(res.mca, ncp = 3, nbelements = Inf)
dimdesc(res.mca)
plot(res.mca, invisible = c("var", "quali.sup"), autoLab = "y", cex = 0.7)
plot(res.mca, invisible = "ind", autoLab = "y", cex = 0.7)
