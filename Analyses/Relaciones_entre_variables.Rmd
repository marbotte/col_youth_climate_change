...
data1 <- data1 %>%
  mutate(imp_sci = case_when(
    imp_scien %in% c("-3", "-2", "-1", "0") ~ "Negativo",
...
data2 <- data1 %>%
  select(Zona, sex, age_r, estrato, etnia, education, edu, kids, ideology, Internet, trust_vote_inst, eco_anx, env_aso, imp_scien, imp_act, imp_rel, imp_pol, thoughts, imp_como, imp_cli, no_car, one_fw_ch, veg, only_bk_wk, clim_origin, cont_clim)

data3 <- data1 %>%
  select(Zona, sex, age_r, strat, ethni, edu, kids, ideology, Internet, trust_vote_inst, eco_anx, env_aso, imp_sci, imp_ac, imp_re, imp_po, thoughts, imp_com, imp_cl, clim_origin, cont_clim)
...
fviz(res.mca, element = "ind", geom = "point", pointsize = 2, hab = "imp_com", alpha = 0.2)
fviz(res.mca, element = "ind", geom = "point", pointsize = 2, hab = "env_aso", alpha = 0.2)
...
res.mca <- MCA(data3, quali.sup = c(1:8), graph = FALSE)
fviz_eig(res.mca, addlabels = TRUE, title = "Inertia of MCA Dimensions")
fviz_mca_ind(res.mca, geom = "point", pointsize = 2, repel = TRUE)
#fviz_mca_biplot(res.mca, geom.ind = "point", repel = TRUE, alpha.ind = 0.1)
fviz_mca_var(res.mca, choice = "var.cat", repel = TRUE)
...
plot_cli_eco_anx <- data2 %>%
  ggplot(aes(x = imp_cli, fill = eco_anx)) +
  geom_bar(position = "fill") +
...
plot_ori_imp_como <- data2 %>%
  ggplot(aes(x = clim_origin, fill = imp_como)) +
  geom_bar(position = "fill") +
  labs(x = "Origen Cambio Climático", y = "Proporción", fill = "Impacto acciones \nen la comodidad") +
  papaja::theme_apa() +
  scale_fill_manual(values = custom_palette)
...
```{r relación origen cambio climatico y redes sociales, fig.align='center', echo=F}
...
```{r logistic regression models, include=FALSE}
